...
class ReferenceDataGenerator:
    ...
    # -------------------------------------------------------
    # INT6022 - Job Classification  [INT6022 — v3 normalization]
    # Old flat format replaced with normalized parent/child structure.
    # 6 fields: Job_Classification_ID (PK), Job_Classification_WID,
    ...
    def _gen_job_classifications(self) -> List[Dict]:
        rows = []
        # Build group lookup: family_group -> (group_id, group_name)
        group_lookup = {}
        group_seq = 1
        for jp in self.job_profiles:
            fg = jp["Job_Family_Group"]
            if fg not in group_lookup:
                group_lookup[fg] = {
                    "id": f"JCG_{group_seq:04d}",
                    "name": jp["Job_Family_Group_Name"],
                }
                group_seq += 1
        ...
        for jp in self.job_profiles:
            fg = jp["Job_Family_Group"]
            grp = group_lookup[fg]
            jc_id = f"JC_{jc_seq:04d}"
            jc_wid = utils.generate_wid(self.rng)
            row = {
                "Job_Classification_ID": jc_id,
                "Job_Classification_WID": jc_wid,
                "Job_Classification_Name": jp["Job_Profile_Name"],
    ...
    # -------------------------------------------------------
    # INT6027 - Matrix Organization  [INT6027 — v3 adds Matrix_Organization_Description]
    # -------------------------------------------------------
    def _gen_matrix_orgs(self) -> List[Dict]:
        rows = []
        for mo in config.MATRIX_ORGS:
            description = (
                f"Cross-functional {mo['type'].lower()} focused on {mo['name']} "
                f"initiatives. Subtype: {mo['subtype']}. Status: {mo['status']}."
            )
            row = {
                ...
                "Matrix_Organization_Description": description,
            }
            rows.append(row)
        return rows
    ...
    def _gen_departments(self) -> List[Dict]:
        ...
        for dept_def in config.TOP_LEVEL_DEPTS:
            ...
            row = {
                ...
                "Owner_EIN_WID": utils.generate_wid(self.rng),  # INT6028 v3: FK placeholder to Worker_Workday_ID
                "Department_Level": 1,
...
FIELD_ORDERS = {
    ...
    "INT6021": [
        # INT6021 v3: field order fixed per spec (Job_Profile_ID first)
        "Job_Profile_ID", "Job_Profile_WID", "Compensation_Grade",
        "Critical_Job_Flag", "Difficult_to_Fill_Flag", "Inactive_Flag",
        "Job_Category_Code", "Job_Category_Name", "Job_Exempt_Canada",
        "Job_Exempt_US", "Job_Family", "Job_Family_Group",
        "Job_Family_Group_Name", "Job_Family_Name", "Job_Level_Code",
        "Job_Level_Name", "Job_Profile_Code", "Job_Profile_Description",
        "Job_Profile_Name", "Job_Profile_Summary", "Job_Title",
        "Management_Level_Code", "Management_Level_Name", "Pay_Rate_Type",
        "Public_Job", "Work_Shift_Required",
        "JOB_MATRIX", "IS_PEOPLE_MANAGER", "IS_MANAGER", "FREQUENCY",
    ],
    "INT6022": [
        # INT6022 v3: normalized parent/child structure (6 fields)
        "Job_Classification_ID", "Job_Classification_WID",
        "Job_Classification_Name", "Job_Classification_Group_ID",
        "Job_Classification_Group_Name", "Job_Profile_ID",
    ],
    "INT6023": [
    ...
    "INT6027": [
        # INT6027 v3: Matrix_Organization_Description added at end
    ...
    "INT6028": [
        # INT6028 v3: Owner_EIN_WID added after Owner_EIN (full 12-field spec)
